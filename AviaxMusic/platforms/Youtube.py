...
import os, string, asyncio, base64, glob
import re, requests, json, random, yt_dlp
...
API_URL = "API_URL=https://tgmusic.fallenapi.fun"
...
class YouTubeAPI:
    ...
    async def download(
        ...
        def audio_dl(vid_id):
            try:
                if not API_KEY:
                    print("𝖠𝖯𝖨 𝖪𝖤𝖸 𝗇𝗈𝗍 𝗌𝖾𝗍 𝗂𝗇 𝖼𝗈𝗇𝖿𝗂𝗀, 𝖲𝖾𝗍 𝖠𝖯𝖨 𝖪𝖾𝗒 𝗒𝗈𝗎 𝗀𝗈𝗍 𝖿𝗋𝗈𝗆 @tgmusic_apibot")
                    return None
                if not API_URL:
                    print("API Endpoint not set in config\nPlease set a valid endpoint for API_URL in config.")
                    return None
                headers = {
                    "x-api-key": f"{API_KEY}",
                ...
                if os.path.exists(xyz):
                    return xyz
                getAudio = requests.get(f"{API_URL}/audio/{vid_id}", headers=headers, timeout=60)
                try:
